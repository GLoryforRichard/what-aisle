 ...
 * live ─(invoice.payment_failed > 7 days)→ suspended
 * suspended ─(invoice.paid)→ live
 * any ─(customer.subscription.deleted)→ suspended
 * any ─(charge.refunded)→ canceled
 *
 * Every handler is idempotent (safe on Stripe event replay) and never
 * throws: the payment bookkeeping in the Stripe provider must not be
 * affected by store provisioning issues. Failures are logged and the
 * founder is notified.
 */

const PAYMENT_FAILED_GRACE_MS = 7 * 24 * 60 * 60 * 1000;
 ...
 */
async function pushStatusToStoresApp(
  slug: string,
  status: StoreStatus
): Promise<void> {
  try {
 ...
 * pending_payment → awaiting_video, record Stripe ids, provision the
 * store in the Stores App, email the video-shooting instructions.
 */
export async function onStoreCheckoutCompleted(
  ...
  try {
    ...
    // Idempotent transition: only a pending_payment row moves forward.
    ...
    if (transitioned.length === 0) {
      ...
      if (existing.length > 0) {
        ...
        console.log(
          `[store-lifecycle] store '${slug}' already provisioned (status: ${existing[0].status}), skipping`
        );
        return;
      }
    ...
    try {
      await createStore({
        slug,
        name: storeName,
        portalUserId: storeUserId ?? userId ?? '',
        stripeCustomerId: customerId,
        subscriptionId,
      });
    } catch (error) {
      console.error(
        `[store-lifecycle] createStore failed for '${slug}':`,
        error
      );
      await notifyFounder(
    ...
    try {
      const email = await resolveCustomerEmail(session, storeUserId ?? userId);
      if (email) {
    ...
    console.error(
      `[store-lifecycle] onStoreCheckoutCompleted error for '${slug}':`,
      error
    );
  }
}
 ...
 * (dunning), bring it back to live and push to the Stores App.
 */
  ...
  try {
    ...
    if (store.status === STORE_STATUS.SUSPENDED) {
      ...
      if (updated.length > 0) {
        ...
        await pushStatusToStoresApp(store.slug, STORE_STATUS.LIVE);
        await notifyFounder(
...
export async function onStoreChargeRefunded(
  ...
  try {
    ...
    if (updated.length > 0) {
      ...
      await pushStatusToStoresApp(store.slug, STORE_STATUS.CANCELED);
      await notifyFounder(
