...
/** Shape of GET /api/debug (see app/api/debug/route.ts). */
interface DebugData {
  ok: boolean;
  store?: string;
  counts?: {
    shelf_evidence: number;
    products: number;
    ...
    <div style={{ padding: '20px 16px 60px', background: '#fff', color: '#111', minHeight: '100vh' }}>
      ...
      {data?.ok && data.counts && (
        <>
          ...
          <Section
            name="search_history"
            count={data.counts.search_history}
            items={data.search_history ?? []}
            summarize={summarizeSearchHistory}
            searchFields={['query', 'product']}
          />
        </>
      )}
    </div>
...
function summarizeSearchHistory(it: Record<string, unknown>): Summary {
  const query = (it.query as string) || '—';
  const found = !!it.found;
  const product = (it.product as string) || '';
  return {
    primary: query,
    secondary: found ? `found${product ? ` · ${product}` : ''}` : 'no match',
    meta: formatTime(it.ts),
  };
}
